...
public class NgrokDownloader(
    ...
    ILogger<NgrokDownloader> logger) : INgrokDownloader
{
    public async Task DownloadExecutableAsync(CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogTrace("Downloading {DownloadUrl}", downloadUrl);
            await DownloadFileAsync(downloadUrl, cancellationToken);
            logger.LogTrace("Downloaded {DownloadUrl}", downloadUrl);
        }
        else
        {
            logger.LogTrace("Ngrok compressed file has already been downloaded.");
        }

        var ngrokFileName = GetExecutableFileName();
        if (!File.Exists(ngrokFileName))
        {
            logger.LogTrace("Extracting {ZipFileName} to {NgrokFileName}", zipFileName, ngrokFileName);
            await ExtractCompressedFileToCurrentDirectoryAsync();
            logger.LogTrace("Extracted {ZipFileName} to {NgrokFileName}", zipFileName, ngrokFileName);
        }
        else
        {
            logger.LogTrace("Ngrok executable already has been extracted.");
        }
    }
